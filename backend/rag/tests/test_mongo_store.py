...
from rag.ingestion import ExtractedImage, PageText, ParsedDocument, TextChunk
from rag.services import MongoBookStore
...
def test_mongo_store_writes_indexed_document_with_full_text(tmp_path: Path):
    ...
    book_id = store.mark_indexed(path=book_path, parsed=parsed, chunks=[chunk])
...
def test_mongo_store_replaces_chunks_for_existing_book(tmp_path: Path):
    ...
    book_id = store.mark_indexed(path=book_path, parsed=parsed_with_image, chunks=[first])
    store.mark_indexed(path=book_path, parsed=parsed, chunks=[second])
