...
from rag.ingestion import ExtractedImage, ParsedDocument, TextChunk, sanitize_document_name
from rag.schemas import SearchHit, Source, Usage
...
class MongoBookStore:
    ...
    def book_id_for_path(self, path: Path) -> str:
        return str(uuid.uuid5(uuid.NAMESPACE_URL, path.resolve().as_posix()))

    def chunk_ids_for_book(self, book_id: str) -> list[str]:
        return [
            str(chunk["_id"])
    ...
    def mark_indexed(
        ...
        *,
        ...
        book_id = self.book_id_for_path(path)
        self._replace_book(
            path=path,
            parsed=parsed,
            book_id=book_id,
    ...
    def mark_error(self, *, path: Path, error: str) -> str:
        book_id = self.book_id_for_path(path)
        book_record = {
            "_id": book_id,
...
def _image_record(image: ExtractedImage, book_id: str) -> dict[str, object]:
    return {
        **image.payload(),
        "_id": image.id,
        "book_id": book_id,
