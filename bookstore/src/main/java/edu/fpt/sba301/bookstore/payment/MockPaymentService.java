...
public class MockPaymentService implements PaymentService {
    ...
    public PaymentResponse createPaymentUrl(Order order, String returnUrl) {
        ...
        StringBuilder paymentUrl = new StringBuilder(baseUrl)
                .append("/api/payment/webhook/mock")
                .append("?orderId=").append(order.getId())
                .append("&amount=").append(order.getTotal())
                .append("&transactionId=").append(transactionId)
                .append("&signature=").append(signature)
                .append("&status=success");

        if (returnUrl != null && !returnUrl.isBlank()) {
            String redirectTarget = returnUrl.replaceAll("/$", "") + "/" + order.getId();
            paymentUrl.append("&returnUrl=")
                    .append(URLEncoder.encode(redirectTarget, StandardCharsets.UTF_8));
        }

        return new PaymentResponse(paymentUrl.toString(), transactionId);
    }
